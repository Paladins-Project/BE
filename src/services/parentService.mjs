...
export const getNumberOfParentByMonthAsync = async (month, year) => {
    try {
        ...
        // Count parents created in the specified month
        ...
        return {
            success: true,
            status: 200,
            message: `Found ${parentCount} parents created in month ${monthNum}/${yearNum}`,
            data: {
                month: monthNum,
                year: yearNum,
                parentCount
        ...
        console.error('Get number of parents by month service error:', error);
        return {
            success: false,
            status: 500,
            message: 'Failed to get number of parents by month',
            error: error.message
        };
    }
};

export const getNumberOfParentByYearAsync = async (year) => {
    try {
        ...
        // Count parents created in the specified year
        const parentCount = await Parent.countDocuments({
            createdAt: {
                $gte: startDate,
                $lt: endDate
        ...
        return {
            success: true,
            status: 200,
            message: `Found ${parentCount} parents created in year ${yearNum}`,
            data: {
                year: yearNum,
                parentCount
            }
        ...
        console.error('Get number of parents by year service error:', error);
        return {
            success: false,
            status: 500,
            message: 'Failed to get number of parents by year',
            error: error.message
        };
    }
}; 